...
| Medium | **`00-environment-check.ipynb` is scarcer than this table said.** The registry names one for six of eight modules and records its absence for `nids-iyp` and `nids-ucsdnt-expanse`. But the two release-1 key repos that are cloned here, `nids-asn-introduction-key` and `nids-bgp-control-plane-key`, **have never held one — no branch, no commit** **[verified 2026-09-04]**, so the earlier "six of eight have one" reading is wrong for at least two of the six and unchecked for the remaining four, whose key repos are private and not cloned here. For IYP the gap is real, since it targets JupyterHub like the others; for UCSDNT it is expected, since the notebook would have to run under Slurm. `nids-setup.py verify` handles this rather than being blocked by it: it reports the discrepancy and runs the module's own key notebook instead, which is the stronger check anyway. **To settle:** clone the other four key repos and look. |
